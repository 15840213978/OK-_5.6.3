...
$Log  = Join-Path $Root "build_tv_arm64_release.log"
...
function Find-BuiltApk {
    ...
    return Get-ChildItem -LiteralPath $base -Recurse -File -Filter "*.apk" |
        Where-Object { $_.FullName -match 'leanback' -and $_.FullName -match 'arm64' -and $_.FullName -match 'release' } |
...
try {
    Banner "OK影视 5.6.3 空壳真MPV / Leanback ARM64 Release"
    Write-Host "[1/3] 检查 MPV 源码、JNI 和原生库..."
    Static-Validate
    Write-Host "      OK：MPV 已内置，不需要再下载 fongmi-media 或 mpv-android。"

    Write-Host "[2/3] 编译 Leanback ARM64 Release..."
    Push-Location $Root
    try {
        & .\gradlew.bat :app:assembleLeanbackArm64_v8aRelease --console=plain --stacktrace 2>&1 | Tee-Object -FilePath $Log
    ...
    $apk = Find-BuiltApk
    if (-not $apk) { throw "编译结束但没有找到 Leanback ARM64 Release APK。" }
